...
# У меня вывод отличался от того, который был в задании. Делал ночью, поэтому пропустил этот момент.
# Теперь вывод соответствует тому, который приведен в задании.
...
# Если при переборе встречается тип данных отличающийся от числового, то обрабатывается исключение TypeError,
# увеличивая счётчик incorrect_data на 1.
            incorrect_data += 1
...
# В случае некорректного типа данных в numbers возвращает None и выводит сообщение об ошибке.
    ...
    try:
# Внутри calculate_average(numbers) для подсчёта суммы используем функцию personal_sum.
...
# Т.к. коллекция numbers может оказаться пустой, то обрабатывается исключение ZeroDivisionError
# при делении на 0 и возвращается 0.
        return 0
    except TypeError:
# В numbers может быть записана не коллекция, а другие типы данных, например числа.
# Обрабатываем исключение TypeError выводя строку 'В numbers записан некорректный тип данных'.
        print('В numbers записан некорректный тип данных')
# Тогда функция просто вернёт None.
        return None
